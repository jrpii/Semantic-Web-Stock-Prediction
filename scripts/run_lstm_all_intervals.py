...
def main():
    ...
    for interval in args.intervals:
        ...
        model_label = "LSTM_NEWS" if run_args.feature_set == "extended_news" else "LSTM"

        m_all = report["best_test_metrics_at_best_epoch"]
        rows.append(_row(f"{interval}m_ALL", model_label, m_all))

        bo = report.get("best_test_breakout_metrics_at_best_epoch")
        if isinstance(bo, dict):
            rows.append(_row(f"{interval}m_BREAKOUTS", model_label, bo))
