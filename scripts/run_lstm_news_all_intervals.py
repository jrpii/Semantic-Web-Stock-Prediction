# Run LSTM (OHLCV + news features) across all bar intervals and write a simple CSV report.
# Output CSV columns match train_evaluate_models.py: dataset, model, roc_auc, f1, precision, recall, accuracy
# Checkpoints are saved by train_lstm_ohlcv.py under analysis_outputs/models/lstm_news/.
import argparse
import sys
...
def main():
    ...
    parser.add_argument("--out-csv", type=Path, default=ROOT / "analysis_outputs" / "reports" / "lstm_news_interval_results.csv")
    ...
    exclude = frozenset({"intervals", "out_csv"})
    rows = []

    for interval in args.intervals:
        print(f"\n--- Interval: {interval} minutes (news features) ---")
        run_cfg = {k: v for k, v in vars(args).items() if k not in exclude}
        run_cfg["interval"] = interval
        run_cfg["feature_set"] = "extended_news"
        run_cfg["save_model"] = True
        run_cfg["out_model"] = None
        run_cfg["balance_loss"] = not run_cfg["no_balance_loss"]
        run_cfg["no_news_merge"] = False
        run_args = argparse.Namespace(**run_cfg)

        report = lstm.run_training(run_args)

        m_all = report["best_test_metrics_at_best_epoch"]
        rows.append(_row(f"{interval}m_ALL", "LSTM_NEWS", m_all))

        bo = report.get("best_test_breakout_metrics_at_best_epoch")
        if isinstance(bo, dict):
            rows.append(_row(f"{interval}m_BREAKOUTS", "LSTM_NEWS", bo))
