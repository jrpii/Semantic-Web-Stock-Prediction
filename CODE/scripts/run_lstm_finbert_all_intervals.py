# Run LSTM (technical + FinBERT sentiment per bar) across all intervals.
# First build news_finbert_by_bar.csv w/ build_finbert_by_bar.py.
import argparse
import sys
...
def main():
    ...
    parser.add_argument("--out-csv", type=Path, default=ROOT / "EVALUATIONS" / "analysis_outputs" / "reports" / "lstm_finbert_interval_results.csv")
    ...
    parser.add_argument("--finbert-by-bar", type=Path, default=lstm.DEFAULT_FINBERT_BY_BAR)
    parser.add_argument(
        "--feature-set",
        choices=("extended_finbert", "extended_finbert_eng", "extended_news_finbert", "extended_news_finbert_eng"),
        default="extended_finbert_eng",
    )
    ...
    for interval in args.intervals:
        print(f"\n--- Interval: {interval} minutes (FinBERT) ---")
        ...
        for t in range(trials):
            ...
            run_cfg["feature_set"] = str(args.feature_set)
            run_cfg["save_model"] = bool(trials <= 1)
            run_cfg["out_model"] = None
            run_cfg["balance_loss"] = not run_cfg["no_balance_loss"]
            run_cfg["news_aligned"] = lstm.DEFAULT_NEWS_ALIGNED
            run_cfg["no_news_merge"] = True
        ...
        if all_avg is not None:
            rows.append(_row(f"{interval}m_ALL", "LSTM_FINBERT", all_avg))

        bo_avg = _avg_metrics(bo_trials)
        if bo_avg is not None:
            rows.append(_row(f"{interval}m_BREAKOUTS", "LSTM_FINBERT", bo_avg))
