...
# Checkpoints are saved by train_lstm_ohlcv.py under EVALUATIONS/analysis_outputs/models/lstm_news/.
import argparse
import sys
...
def main():
    ...
    parser.add_argument("--out-csv", type=Path, default=ROOT / "EVALUATIONS" / "analysis_outputs" / "reports" / "lstm_news_interval_results.csv")
    ...
    for interval in args.intervals:
        ...
        if all_avg is not None:
            rows.append(_row(f"{interval}m_ALL", "LSTM_NEWS", all_avg))

        bo_avg = _avg_metrics(bo_trials)
        if bo_avg is not None:
            rows.append(_row(f"{interval}m_BREAKOUTS", "LSTM_NEWS", bo_avg))
